...
const MintMusic = () => {
  ...
  const sendTxToBlockchain = async (metaData) => {
    try {
      setTxStatus("connecting to XDC Blockchain.");
      ...
      setErrorMessage("Failed to send tx to XDC.");
      console.log(error);
    }
  };
  ...
  const previewNFT = (metaData, mintNFTTx) => {
    ...
    setTxURL(`https://explorer.apothem.network/tx/${mintNFTTx.hash}`);
    setTxURL(`https://explorer.xinfin.network/tx/${mintNFTTx.hash}`);
